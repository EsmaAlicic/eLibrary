...
export const deleteUser = (req,res) => {
    ...
    jwt.verify(token,"unama", (err, userInfo)=>{
        ...
        if (!userInfo.isAdmin) {
            return res.status(403).json("You do not have permission for this operation!");
        }
        
        const q = "DELETE FROM users WHERE `id` = ? ";

        db.query(q,[userInfo.id], (err, data) =>{
            if(err) return res.status(403).json("You do not have permission for this operation!BE GONE YOU LOSER !!!")
